...
abstract class AbsFadeInView(private val context: Context) : PopupView, View.OnClickListener {
    ...
    private fun viewInit() {
        ...
        contentView.let {
            ...
            run {
                background.layoutParams = ViewGroup.LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT, ViewGroup.LayoutParams.MATCH_PARENT)
                background.setBackgroundColor(Color.parseColor("#80000000"))
                background.alpha = 0.0f
                background.setOnClickListener(this)
                ((context as Activity).findViewById<View>(android.R.id.content) as? ViewGroup)!!.addView(background)

            }

            it.layoutParams = ViewGroup.LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT, ViewGroup.LayoutParams.MATCH_PARENT)
            it.alpha = 0.0f
            ((context as Activity).findViewById<View>(android.R.id.content) as? ViewGroup)!!.addView(it)
        }
    ...
    fun show(duration: Long, interpolator: TimeInterpolator) {
        ...
        contentView.animate().alpha(1.0f).setDuration(duration).setListener(object : Animator.AnimatorListener {
            override fun onAnimationRepeat(animation: Animator?) {
            }

            override fun onAnimationEnd(animation: Animator?) {
                this@AbsFadeInView.setData()
            }
            ...
            override fun onAnimationStart(animation: Animator?) {
                this@AbsFadeInView.preAnimation()
            }
    ...
    fun dimiss(duration: Long) {
        ...
        val animator = ObjectAnimator.ofFloat(contentView, "alpha", 1.0f, 0.0f)
        animator.apply {
        ...
        animatorSet.addListener(object : Animator.AnimatorListener {
            ...
            override fun onAnimationEnd(animation: Animator?) {
                ((context as Activity).findViewById<View>(android.R.id.content) as? ViewGroup)!!.removeView(background)
                ((context as Activity).findViewById<View>(android.R.id.content) as? ViewGroup)!!.removeView(contentView)
            }
